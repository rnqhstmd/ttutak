...
3. **신규 기능 추가 시**: 비슷한 패턴의 기존 구현이 있으면 읽어서 프로젝트의 관례를 파악한다.
   - 신규 API 추가 → 기존 API 하나의 전체 흐름(라우팅→서비스→저장소)을 추적.
...
**규칙:**
- 기술 선택지(JWT vs 세션, REST vs GraphQL 등)는 `유형: 선택`으로 표시한다.
- 구체적 설정값이나 자유 서술이 필요한 질문은 `유형: 자유입력`으로 표시한다.
- 모든 선택형 질문에 `(권장)` 표시가 있는 선택지를 하나 포함하고, 권장 근거를 간략히 명시한다.
- 모든 선택형 질문의 마지막 선택지는 `직접 입력`으로, 사용자가 자유 텍스트를 입력할 수 있도록 한다.
- 각 선택지에 기술적 트레이드오프를 간략히 명시한다.
